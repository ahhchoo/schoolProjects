...
public class Controller {
	...
	@FXML protected Text failTarget, successTarget, missingTarget; 
	@FXML protected TextField tfUrl,tfPort, tfSchema, tfUser;
	@FXML protected PasswordField tfPass; 
	@FXML protected ChoiceBox<String> choiceDB;
	@FXML protected Button btnSubmit;         
	protected static WelcomeModel welcome = null; 
	protected static DatabaseModel db = null; 
	...
	//Choice Login
	@FXML protected void handleChoiceBoxAction(Event event){
	...
	//Function to login 
	@FXML protected void handleLogin() throws SQLException {
	...
	protected boolean validPort(int num){
		if(num < 1 || num > 65535){
			return false; 
		} else return true; 
	}
...
@FXML protected void handleCreateAction(){
	...
	//determines whether information matches Database entries
/*
 * protected boolean isRightType(String[] arr){//VARCHAR //INTEGER
		String[] colType = null;
		...
		for(int i =0; i <arr.length; i++){
			if(colType[i].equals("NUMBER") || colType[i].equals("INT")){
 ...
 */
	protected boolean isRightType(String[] arr){//VARCHAR //INTEGER
		...
		for(int i =0; i <arr.length; i++){
			System.out.println("Column Type is " + colType[i]);
			if(colType[i].equals("INT") || colType[i].equals("NUMBER") ){
				numExist = arr[i].matches("\\d+");
				System.out.println(numExist);
			}
		}
		return numExist; 
	}
	...
	@FXML protected void handleDisplayAllAction() throws SQLException{
	...
	@FXML protected void handleSearchAction(){
		search = new GridPane();
		tabSearch = new Tab();
				...
				public void handle(ActionEvent event) {
					...
					for(int i =0; i < cbGetInfo.length; i++){
						...
						if(cbGetInfo[i].isSelected()){
							System.out.println("CBGETINFO SELECTED" + cbGetInfo[i].getText());
							count++; 
					...
					if(count >0){
						...
						for(int i =0, j=0; i <cbGetInfo.length;i++){
							if(cbGetInfo[i].isSelected()){
								...
								System.out.println("COLUMNS BEING STORED INTO ARRAY" + colName[i]); 
								j++; 
							}
						}
					}else{
					...
					try {
						...
						if(finds[0][0]==null){
							...
							for(int i=0; i <finds.length;i++){//row num
								for(int j=0; j<finds[i].length; j++){//col num
									if(finds[i][j]==null){
										break;
									}else{
										System.out.println(finds[i][j]);
									}
								}
							}
						}
	...
	//not sure 
    @FXML protected void handleUpdateAction() throws SQLException{
    	update = new GridPane();
    ...
    @FXML protected void handleDeleteAllAction(){
	...
	//LOGOUT OF DATABASE
	@FXML protected void handleLogoutAction(){
		try {
